<template>
    <v-main class="list">
        ...
        <v-card>
            <v-data-table :headers="headers" :items="todos" :search="filterData" :expanded="expanded" 
                            item-key="task" show-expand v-model="checkbox" show-select >   
                <template v-slot:[`item.priority`]="{ item }">
                    <v-icon v-show="item.priority=='Pentingg'" color="red">mdi-alert-octagon</v-icon>
                ...
                <template v-slot:expanded-item="{ headers, item }">
                    <td v-if="item.priority=='Pentingg'" class="red darken-2" :colspan="headers.length">
                        <v-chip
                        color="red"
                        text-color="white">
            ...
            <v-card>
                ...
                <v-card-text>
                    <v-container>
                        ...
                        :items="[`Pentingg`, `Biasa`, `Tidak Penting`]"
                        label="Priority"
                        required></v-select>
        ...
        <v-dialog v-model="dialogDelete" persistent max-width="600px">
            ...
            <v-card v-if="checkbox.length != 0">
                <v-card-title class="justify-center">
                        <b>DELETE MULTIPLE</b>
                </v-card-title>

                <v-card-text v-for="(checkbox,item) in checkbox" :key="item">        
                        {{ checkbox.task }}
                </v-card-text>

                <v-footer>
                        <v-btn class="mx-auto" style="color: white; background-color: #FF0000;" @click="deleteSelectedItem()">HAPUS SEMUA</v-btn>
                </v-footer>
            </v-card>
...
export default{
    ...
    computed:{
        filterData(){
            ...
            if(this.filter=="Penting"){
                return "Pentingg"
            }
            if(this.filter=="Tidak Penting"){
    ...
    methods: {
        ...
        deleteSelectedItem(){
            this.temp = this.checkbox.length;

            while(this.temp!=0){
                for(var i = 0; i<this.checkbox.length; i++){
                    for(var j = 0; j<=this.todos.length; j++){
                        if(this.checkbox[i]==this.todos[j]){
